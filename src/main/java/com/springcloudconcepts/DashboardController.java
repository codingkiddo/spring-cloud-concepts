...
public class DashboardController {
	...
	public DashboardController( @Value("${custom.bucket-name}") String  bucketName, S3Client s3Client) {
...
//	@PostConstruct
	public void postConstruct() {
		this.bucketLocation = String.format("https://%s.s3.%s.amazonaws.com", 
				bucketName, this.s3Client.getBucketLocation(GetBucketLocationRequest.builder().bucket(bucketName).build()));
	}
